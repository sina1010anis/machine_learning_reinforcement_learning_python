...
x_tr, x_te, l_tr, l_te = train_test_split(data_normal, data_label, test_size=0.3, random_state=42, stratify=data.status) # تقسیم داده ها برای تست و اموزش 

knn = KNeighborsClassifier(n_neighbors=3) # کانفیگ روش knn

knn.fit(x_tr, l_tr) # فیت کردن داده و برچسب ها

l_pre = knn.predict(x_te)

print('Score =>',knn.score(x_te, l_te))

print('Confusion Matrix =>','\n',confusion_matrix(l_te, l_pre))

# print(classification_report(l_te, l_pre))
