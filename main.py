...
# x_tr, x_te, l_tr, l_te = train_test_split(data_x, label, test_size=0.3, random_state=True)
...
# l_pre_pro = lr.predict_proba(x_te)[:,1]

# roc = roc_auc_score(l_te, l_pre_pro)

# print(roc)

# ########################################### نمایش درصدی TP, TN, FP, FN

# cm = confusion_matrix(l_te, l_pre)

# cm = normalize(cm, norm='l1', axis=1)

# cm_df = pd.DataFrame(cm, columns=['B', 'M'], index=['B', 'M'])
...
#     kmn = KMeans(n_clusters=i+1) # روش k-means است با وردی اول ما تعداد خئ=وشه ها را مشخص میکنیم

#     kmn.fit(data_x)

#     lab = kmn.predict(data_x)

#     c.append(kmn.cluster_centers_) # با این مقدار مرکز هر خوشه مشخص میشود

#     labels.append(kmn.inertia_) # با این مقدار امتیاز هر تعداد از خوشه مشخص میشود

#     scores.append(kmn.score(data_x)) 

#     plt.subplot(3, 3, i+1)

#     plt.scatter(data_x[:,2], data_x[:,3], c=lab)
    
#     plt.scatter(kmn.cluster_centers_[:,2], kmn.cluster_centers_[:,3], marker='x', c='red')
